    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitswall: #没有撞墙则可以继续移动
                ...
                successor = ((nextnode, cornerlist), action, 1)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    hn = minmanhattan(unvisitedcorner, currentpos)  # 计算所有未经过的角落的到现在距离的最小曼哈顿值，一定比实际距离小
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    val=[]
    for tmp in food_cor:
        distance=mazeDistance(tmp,position,problem.startingGameState)  # 当前点到所有食物点中地图步数最大的距离
        val.append(distance)
    return max(val)
